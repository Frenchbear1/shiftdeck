...
test("searches all references, condenses contacts, and marks GSC workers", async () => {
  ...
  assert.match(page, /referenceVault\.emailDirectory/);
  assert.match(page, /className="panel contact-group email-directory"/);
  assert.match(page, /<details className="panel contact-group" key=\{group\.title\}>/);
  assert.match(page, /href=\{`mailto:\$\{entry\.email\}`\}/);
  ...
  assert.match(css, /\.contact-methods\s*\{[\s\S]{0,180}?grid-template-columns: minmax\(0, 1fr\)[\s\S]{0,180}?justify-self: end/);
  assert.match(css, /\.email-directory-list\s*\{/);
